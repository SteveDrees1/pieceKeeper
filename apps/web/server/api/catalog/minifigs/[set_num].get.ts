...
export default defineEventHandler(async (event) => {
  ...
  const url = `${REBRICKABLE_BASE}/minifigs/${encodeURIComponent(setNum)}/?key=${encodeURIComponent(key)}`;
  const res = await fetch(url, { headers: { Accept: "application/json" } });

  if (!res.ok) {
    if (res.status === 404) {
      throw createError({ statusCode: 404, statusMessage: "Minifig not found" });
    }
